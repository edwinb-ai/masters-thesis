...
the sole purpose of NN and ML methods in general: \emph{to learn a mapping or function}. 
Assuming it is possible to have a training set \(\mathcal{D}\) with enough examples, 
containing both inputs and outpus, the function that determines the relationship between 
...
expected to change. Now, there are several questions that need an answer in this figure. 
First, what is the proportion between the change in the weights to the change in the 
...
Here, only the main results will be reproduced and mentioned. In optimization, specifically \emph{unconstrained optimization}, the task is to find the value of an input vector \(\mathbf{x}\) that \emph{minimizes the function} \(f\), such that \(f(\mathbf{x}) = 0\). The fact that it is unconstrained is because there is no restriction in the bounds of \(\mathbf{x}\), or in the form of \(f\), which in general is a non-linear function. In the context of optimization, \(f\) is referred to as a cost function, or objective function.
Now, a relation between optimization and NN can be made by noting the following fact. The purpose of NN is to learn the function that relates inputs and outputs in a given data set. Or in the context of optimization: \emph{to minimze the error between the true values and the approximation values}. The \emph{true values} correspond to the outputs in the data set, and the \emph{approximation values} to the outputs from the NN. So, in essence, the task is to find \emph{the best values} of the weights, \(w_j\)
