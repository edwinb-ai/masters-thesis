...
\end{equation}
with $K$ being the kinetic energy and $U$ the potential energy of the system.
...
\emph{phase space vector} $\Gamma \left( \vecr, \vecp \right)$, however, considering that a conservative system where all
...
at temperature $T$ that can exchange energy with a heat bath.~\cite{mcquarrieStatisticalMechanics2000} This ensemble has a
...
\emph{de Broglie} wavelength.~\cite{mcquarrieStatisticalMechanics2000} This again shows a correspondence between classical
...
\begin{equation}
    \frac{\chi_{T} \, \rho}{\beta} = 1 + \rho \int d \vecr \, \left[ g(r) - 1 \right]
    \; .
    \label{eq:chi-rdf}
\end{equation}
...
It describes the excluded volume interaction of particles, similar to what
...
kinds of computer simulation algorithms with a finite time step. This new continuous function is used
...
\begin{equation}
    ...
    \label{eq:t-star}
\end{equation}
and for a three-dimensional system the parameters used are \(a=1.4543, b=1.199, c=1.0545 .\)

\section{Computer simulations of liquids}

Liquid State Theory is usually supplemented with results from computer simulations
...
In general, Monte Carlo methods are random in nature. However, liquids cannot be
...
for at least 50\% of the configurations sampled, which would incur in numerical instabilities and observables
...
the case that different explorations using different seed for the random number generator
are enough to obtain good estimates for an observable. In this work, Monte Carlo computer 
...
\begin{equation}
    \frac{\beta}{\rho \chi_{T}} = 1 - \rho \int c(r) \, d \vecr
    \, ,
...
consistency will be explored in detail later in this work.
